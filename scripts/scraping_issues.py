    ...
    while True:
        ...
        async with session.get(url, headers=HEADERS) as response:
            if response.status == 200:
                ...
                print(
                    f"❌ Failed to fetch issues: {response.status}, {await response.text()}"
                )
                break
    return issues
    ...
    if "events_url" in issue:
        async with session.get(issue["events_url"], headers=HEADERS) as response:
            if response.status == 200:
                ...
                print(
                    f"❌ Failed to fetch events for issue {issue['number']}: {response.status}"
                )
        ...
        "events": [
            {
                ...
                "comment": (
                    event.get("body", "") if event.get("event") == "commented" else ""
                ),
            }
            for event in events
        ],
